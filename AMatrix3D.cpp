...
// b=this*a
// NOTE: This is an optimized 3x4 that pretends its 4x4
void AMatrix3D::mult(float *a,float *b)
{
  //DEBUG_OUT<<"AMatrix3D::mult()\n";
  b[0]= xx * a[0] + yx * a[1] + zx * a[2] + xo;
  b[1]= xy * a[0] + yy * a[1] + zy * a[2] + yo;
  b[2]= xz * a[0] + yz * a[1] + zz * a[2] + zo;
}
...
void AMatrix3D::transformVI(AVertexF3D *v, AVertexI3D *tv, int nvert)
{
  //DEBUG_OUT<<"AMatrix3D::transformVI(,,"<<nvert<<")\n";
  if(!v) return;
  if(!tv) return;
...
void AMatrix3D::transformVF(AVertexF3D *v, AVertexF3D *tv, int nvert)
{
  //DEBUG_OUT<<"AMatrix3D::transformVF(,,"<<nvert<<")\n";
  if(!v) return;
  if(!tv) return;
  float lxx = xx, lxy = xy, lxz = xz, lxo = xo;
  float lyx = yx, lyy = yy, lyz = yz, lyo = yo;
...
void AMatrix3D::transformI(float *v, int *tv, int nvert)
{
  //DEBUG_OUT<<"AMatrix3D::transformI(,,"<<nvert<<")\n";
  if(!v) return;
  if(!tv) return;
  float lxx = xx, lxy = xy, lxz = xz, lxo = xo;
  float lyx = yx, lyy = yy, lyz = yz, lyo = yo;
...
void AMatrix3D::transformF(float *v, float *tv, int nvert)
{
  //DEBUG_OUT<<"AMatrix3D::transformF(,,"<<nvert<<")\n";
  if(!v) return;
  if(!tv) return;
  float lxx = xx, lxy = xy, lxz = xz, lxo = xo;
  float lyx = yx, lyy = yy, lyz = yz, lyo = yo;
